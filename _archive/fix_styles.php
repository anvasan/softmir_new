...
// Find the marker where we want to cut
$marker = '/* ========== Categories Cards Grid (Homepage) ========== */';
$pos = strpos($content, $marker);

if ($pos !== false) {
    // Keep everything up to the marker + marker length
    // Actually, looking at the previous output, the corruption started RIGHT after this marker.
    // So we can keep up to the closing brace before it? 
    // Wait, the marker is a comment.
    // Let's assume the markers are valid.

    // We want to keep the marker line.
    $valid_content = substr($content, 0, $pos + strlen($marker));

    // Clean up any trailing weirdness if necessary, though substr should be binary safe.
    $valid_content = trim($valid_content) . "\n\n";
...
CSS;
    ...
    echo "Successfully fixed style.css!";
}
else {
